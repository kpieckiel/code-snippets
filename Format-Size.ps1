function Format-Size() {
    <#
    .SYNOPSIS
    Convert a number specified in bytes to another unit (e.g.,
    kilobytes, megabytes, etc.).

    .DESCRIPTION
    This function converts a number in bytes to another unit (e.g.,
    kilobytes, megabytes, etc.).  The resulting unit is automatically
    determined unless explicitly overridden by the -Unit parameter.
    Automatic unit detection will choose the largest unit that can
    ...
    .PARAMETER Unit
    The resulting unit of the conversion.  May be 'Auto' to pick the
    largest unit that can represent the number as a number less than
    1024.

    .PARAMETER NoGrouping
    Suppress including a grouping character (e.g., comma or period) in
    the number when output.
    ...
    Fewer than this number of digits may be displayed if full precision
    is reached.

    .INPUTS
    System.Numerics.BigInteger.  This function accepts an unsigned
    integer that is to be conversion.

    .OUTPUTS
    System.String.  This function returns the input number converted to
    the specified unit
    with a unit identifier.

    .NOTES
    This function is an adaptation of the Format-Size() function
    provided by Theo in a Stack Overflow post.  See related link.
